...
export const POST = async (request: Request, context: RouteContext<'/api/characters/[id]/knowledge/index'>) => {
  const id = (await context.params).id;
  if (!characterRepository.findById(id)) return NextResponse.json({ error: '角色不存在' }, { status: 404 });
  try {
    return NextResponse.json({ index: await embeddingIndexService.rebuild(id, request.signal) });
  } catch (error) {
