...
H3 模式适合需要明显语气的重点短句。点击回复下方的“生成情感语音”，CharacterOS 会抽取一条不超过 16 个字的对白，通过本机 ComfyUI MiniMax H3 工作流异步生成独立音轨。为缩短等待，隐藏画面采用 512×288、约 3 秒、12 步配置，并且跳过视频解码与保存；当前配置针对 RTX 5060 8GB 显存优化。
