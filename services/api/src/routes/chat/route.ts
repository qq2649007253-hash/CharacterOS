...
import { chatRequestSchema } from '@characteros/contracts/chat';
...
import { createOllamaTextStream } from '@/server/services/ollamaTextStream';
...
export const POST = async (request: Request) => {
  ...
  if (!parsed.success) return Response.json({ error: '聊天请求不合法', issues: parsed.error.issues }, { status: 400 });
  ...
  if (!conversation) return Response.json({ error: '会话不存在或不属于该角色' }, { status: 404 });
  ...
  try {
    ...
    const textStream = createOllamaTextStream(ollamaStream, {
      ...
      onError(error) {
        agentRunRepository.finish(agentRun.id, 'failed', startedAt, {
          error: error instanceof Error ? error.message : '模型回复中断或请求已取消',
          knowledgeHits: knowledge.length,
          memoryHits: relevantMemories.length,
          retrievalMethod: knowledge[0]?.method || 'none',
          toolCallId: activeToolCallId,
