...
import { toolApprovalSchema, type ToolName } from '@characteros/contracts/tool';
...
export const PATCH = async (request: Request, context: { params: Promise<{ id: string }> }) => {
  ...
  if (!toolCall) return Response.json({ error: '工具调用不存在' }, { status: 404 });
  if (toolCall.status !== 'pending') return Response.json({ error: '该工具调用已经处理' }, { status: 409 });
  const parsed = toolApprovalSchema.safeParse(await request.json());
  if (!parsed.success) return Response.json({ error: '审批请求不合法' }, { status: 400 });
  ...
  if (parsed.data.action === 'reject') {
    ...
    return Response.json({ toolCall: updated });
  }
  ...
  try {
    ...
    return Response.json({ result, toolCall: updated });
  } catch (error) {
    ...
    return Response.json({ error: message, toolCall: updated }, { status: 500 });
  }
};
